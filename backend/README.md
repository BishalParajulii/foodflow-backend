# FoodFlow — Backend (Django + DRF)

Production-oriented food delivery platform API. This directory is the
**backend** of the FoodFlow monorepo — run all commands below from here
(`cd backend`). The repo root holds `docker-compose.yml` (full stack) and
...
```text
foodflow/
├── docker-compose.yml      # full stack (repo root): db + redis + backend + frontend
├── backend/                # <-- you are here (Django project root)
│   ├── manage.py
│   ├── pyproject.toml
│   ├── README.md
│   ├── Dockerfile
...
All commands below run from this directory (`cd backend`).
...
Full stack (run from the **repo root**, not `backend/`):
`web` (Gunicorn + Uvicorn ASGI) + `db` (Postgres 16) + `redis` (Redis 7) +
...
- No env file is required. `docker-compose.yml` (repo root) ships working local
  defaults (production settings with `SECURE_SSL_REDIRECT=False`,
  Postgres/Redis on the compose network). The local `backend/.env` file is for
  `manage.py`/pytest runs only and never affects containers. To override
  container env (`SECRET_KEY`, `GOOGLE_CLIENT_ID`, …), copy
  `backend/.env.docker.example` to `backend/.env.docker` (gitignored); infra
  addresses (`DATABASE_URL`, `REDIS_URL`, …) are set in `docker-compose.yml` itself.
